...
class TradingTasks:
    ...
    Ready-to-use tasks for common trading operations including:
    - Auto historical data fetching
    - Auto live data updates
    ...
    async def fetch_historical_data(self):
        ...
        try:
            from app.data.angel_one_data import get_real_historical_data, AngelOneDataFetcher
            ...
            for symbol in symbols:
                try:
                    # Fetch 1 day of 5m candles
                    candles = get_real_historical_data(symbol, '5m', 1)
                    ...
                    if candles:
                        ...
                        try:
                            ...
                            db.commit()
                            # Clear cache
                            get_cache().delete(f"candles:{symbol}:5m")
                        except Exception as e:
            ...
            self._symbols_fetched = len(symbols) - failed
            
            logger.info(f"✅ Historical data fetch complete: {total_stored} candles stored for {self._symbols_fetched} symbols")
    ...
    async def fetch_live_quotes(self):
        """
        Fetch live quotes for all tracked symbols
        
        Runs every 30 seconds to:
        - Get real-time prices from Angel One
        - Update in-memory cache
        """
        logger.debug("🔄 Fetching live quotes...")
        ...
        try:
            ...
            # Cache the quotes
            if quotes:
                get_cache().set("live_quotes", quotes, ttl=60)
            
            logger.debug(f"✅ Live quotes fetched: {len(quotes)} symbols")
            
            return {
                "task": "fetch_live_quotes",
                "status": "completed",
            ...
            logger.error(f"Live quotes fetch error: {e}")
            return {
                "task": "fetch_live_quotes",
                "status": "failed",
                "error": str(e)
    ...
    async def market_open_setup(self):
        ...
        try:
            ...
            if ao.can_connect():
                ao.login()
                logger.info("✅ Angel One connected for trading session")
        except Exception as e:
    ...
    def register_all_tasks(self):
        ...
        self.scheduler.register_intraday(self.intraday_monitoring, 1)  # Every 1 minute
